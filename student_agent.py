...
def my_state(obs):
    ...
    destination_to_loc = {'R': (Rx, Ry), 'G': (
        Gx, Gy), 'Y': (Yx, Yy), 'B': (Bx, By)}
...
def get_action(obs):
    direction, _, obstacle_north, obstacle_south, obstacle_east, obstacle_west = my_state(
        obs)
    ...
    state = str((direction, obstacle_north, obstacle_south,
    ...
    if state not in policy_table.keys():
        ...
        for x, y in itertools.product(range(-4, 4), repeat=2):
            if x * direction[0] < 0 or y * direction[1] < 0:
                continue
            ns = ((x, y), obstacle_north, obstacle_south,
                  obstacle_east, obstacle_west)
